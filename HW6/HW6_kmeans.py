...
def createGIF(images, gif_name, iter):
    ...
    images[0].save("output/kernel_kmeans_"+str(pic)+"_"+str(args.k)+"_"+mode+".gif", save_all=True,
                   append_images=images[1:], optimize=False, loop=0, duration=100)
    images[0].save("output/kernel_kmeans_"+str(pic)+"_"+str(args.k)+"_"+mode+"0.png")
    images[iter].save("output/kernel_kmeans_"+str(pic)+"_"+str(args.k)+"_"+mode+str(iter)+".png")
    return
...
def initialCluster(x, num_of_cluster, kernel, mode):
    """
    initialize cluster
    mode 1 for first randomly choose centers
    mode 2 for k-means++
    """
    images = []
...
def KernelKmeans(num_of_cluster, cluster, kernel, images):
    ...
    for i in range(1, 100):
        ...
        if np.linalg.norm((new_cluster - current_cluster), ord=2) < 1e-2:
            break
...
if __name__ == "__main__":
    ...
    print("==========K-means==========")
    # initial clustering
    init_cluster, images = initialCluster(
        data_points, args.k, gram_matrix, args.mode)
